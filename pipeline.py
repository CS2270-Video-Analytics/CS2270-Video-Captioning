...
class VideoQueryPipeline():
    ...
    def __init__(self):
        ...
        self.vector_db = VectorDBInteface(d = self.captioning_pipeline.clip_model.visual.output_dim)
    ...
    def process_video(self, video_path:str, video_filename:str):
        ...
        for (sql_batch, vector_batch) in frame_iterator():
            ...
            vector_batch = torch.stack(vector_batch, dim=0)
            self.vector_db_interface.insert_many_vectors(vectors = vector_batch)
        ...
        for (video_id, frame_id, caption) in obj_iterator():
