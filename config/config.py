...
class Config:
    ...
    text2table_frame_prompt = "Given the following description of an image and the set of objects below, output a structured SQL table with the schema [{formatted_schema}]. Separate rows of the SQL table with <n> and elements in a column with <t>. Provide detailed attribute description; where attribute is not present for an object, leave blank. Provide detailed action description; where action is not relevant for for an object, leave blank.\n\nDetailed image description:{image_caption}\n\nSet of objects:{object_set}"
